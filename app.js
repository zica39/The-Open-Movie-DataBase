const omdb =  'http://www.omdbapi.com/?apikey=7acf7772&'

$(document).ready(function(){
	
	
	$('#pretraga').on('search',e => {$('#trazi').trigger('click');$('#pretraga').blur();});
	$('#tip').on('change',e => {$(lista).empty();});
	...
	$('#pretraga').on('input',e =>{
		...
		$.ajax({url: omdb+$.param(pretraga), type:'GET', success: function(result){
	...
	$('#trazi').click((e)=>{
		...
		$.ajax({url: url, isFilm: $('#tip').val() == 'movie' , type:'GET', success: function(result){
